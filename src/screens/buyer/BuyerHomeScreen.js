...
const CATEGORIES = [
  ...
  {id: '4', name: 'Phones', image: utils.assets.onboarding_bid},
...
export default function BuyerHomeScreen({navigation}) {
  ...
  const renderCategory = ({item}) => {
    ...
    return (
      <TouchableOpacity
        activeOpacity={0.8}
        onPress={() => setSelectedCategory(active ? 'All' : item.name)}
  ...
  <SafeAreaView
    ...
    <ScrollView
      ...
      <View className="px-5 pt-4">
        ...
        <View
          className="flex-row items-center border rounded-xl mt-5 px-3 py-1"
          style={{borderColor: utils.colors.lightGrey}}>
          <Search
            size={20}
